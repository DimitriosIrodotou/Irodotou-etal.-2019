...
void check_disk_instability (int p, double dt, double time) {
	...
	double Mcrit, UnstableMass, GasMass, GasRd, DiskMass, DiskRd;
	double fraction, Qgas, Qstar, Qmin, Qtot, W;
#ifdef H2_AND_RINGS
...
#ifdef DI_INSTABILITIES
	W        = 0.5033; // W = 2s / 1+s^2 where s = 0.27
	Qmin     = 1.0 + W;
	GasMass  = Gal[p].ColdGas;
	DiskMass = Gal[p].DiskMass;
	GasRd    = Gal[p].ColdGasRadius / 3.;
	DiskRd   = Gal[p].DiskRadius / 3.;

	// Calculate Q parameters for gaseous and stellar disks.
	if (Gal[p].Type != 0)
	{
		Qgas  = Gal[p].InfallVmax / sqrt (G * GasMass / GasRd);
		Qstar = Gal[p].InfallVmax / sqrt (G * DiskMass / DiskRd);
	}
	else
	{
		Qgas  = Gal[p].Vmax / sqrt (G * GasMass / GasRd);
		Qstar = Gal[p].Vmax / sqrt (G * DiskMass / DiskRd);
	}

	//	Check which disk has larger Q since RW11 formula gives less weight to that component.
	if (Qgas < Qstar)
	{
		Qtot = (Qstar * Qgas) / (Qstar + W * Qgas);
	}
	else
	{
		Qtot = (Qstar * Qgas) / (W * Qstar + Qgas);
	}

	// Check the stability of the two-component disk. Follow RW11 calculations and assume that Qgas/Qstar ~ 1
	// which suggests that, on average, stars and gas contribute equally to the gravitational instability of the disc.
	// So we can define Qmin = 1 + W as a solution to the inequality Qgas * Qstar / Qgas + Qstar >= 1.
	if (Qtot < 1.0)
	{
		// If gaseous disk is stable (i.e., Qgas > Qmin) then Qstar solely has to raise Qtot to 1.
		if (Qgas > Qmin)
		{
			// Calculate disk's critical mass.
			if (Gal[p].Type != 0)
			{
				Mcrit = (Gal[p].InfallVmax * Gal[p].InfallVmax * DiskRd * (Qgas - W) * (Qgas - W)) / (G * Qgas * Qgas);
			}
			else
			{
				Mcrit = (Gal[p].Vmax * Gal[p].Vmax * DiskRd * (Qgas - W) * (Qgas - W)) / (G * Qgas * Qgas);
			}
		...
		// If stellar disk is stable (i.e., Qstar > Qmin) then Qgas solely has to raise Qtot to 1.
		else if (Qstar > Qmin)
		{
			// Calculate disk's critical mass.
			if (Gal[p].Type != 0)
			{
				Mcrit = (Gal[p].InfallVmax * Gal[p].InfallVmax * GasRd * (Qstar - W) * (Qstar - W)) / (G * Qstar * Qstar);
			}
			else
			{
				Mcrit = (Gal[p].Vmax * Gal[p].Vmax * GasRd * (Qstar - W) * (Qstar - W)) / (G * Qstar * Qstar);
			}
			...
			if (UnstableMass > 0.0)
			{
				fix_gaseous_disk_instability (p, GasMass, UnstableMass, fraction, dt, time);
			}
			...
			if (Gal[p].Type != 0)
			{
				Mcrit = (Gal[p].InfallVmax * Gal[p].InfallVmax * DiskRd * (W - Qstar) * (W - Qstar)) / (G * W * W * Qstar * Qstar);
			}
			else
			{
				Mcrit = (Gal[p].Vmax * Gal[p].Vmax * DiskRd * (W - Qstar) * (W - Qstar)) / (G * W * W * Qstar * Qstar);
			}
			// Calculate the unstable stellar mass.
			UnstableMass = DiskMass - Mcrit;
			fraction     = UnstableMass / DiskMass;
			...
			// Both disks are unstable so first raise Qgas to Qmin and then Qstar to Qmin.
		else if (Qstar < Qmin && Qgas < Qmin)
		{
			// Calculate disk's critical mass.
			if (Gal[p].Type != 0)
			{
				Mcrit = (Gal[p].InfallVmax * Gal[p].InfallVmax * GasRd) / (G * Qmin * Qmin);
			}
			else
			{
				Mcrit = (Gal[p].Vmax * Gal[p].Vmax * GasRd) / (G * Qmin * Qmin);
			}
			// Calculate the unstable gaseous mass.
			UnstableMass = GasMass - Mcrit;
			// Invoke a function to deal with the unstable gas.
			...
			if (Gal[p].Type != 0)
			{
				Mcrit = (Gal[p].InfallVmax * Gal[p].InfallVmax * DiskRd) / (G * Qmin * Qmin);
			}
			else
			{
				Mcrit = (Gal[p].Vmax * Gal[p].Vmax * DiskRd) / (G * Qmin * Qmin);
			}
			// Calculate the unstable stellar mass.
			UnstableMass = DiskMass - Mcrit;
			fraction     = UnstableMass / DiskMass;
...
void fix_gaseous_disk_instability (int p, double GasMass, double UnstableMass, double fraction, double dt, double time) {
