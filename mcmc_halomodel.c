...
void halomodel (double*r_arr, double*proj_arr, float masslimit_low, float masslimit_high, int snap) {
	...
	for (i      = 0; i < NK + extralin; i ++) {
		if (i > 2 * extralin) {
			k = pow (10., (i - extralin) * kbin + kstart);
		} else {
			k = pow (10., i * 0.5 * kbin + kstart);
		}
		...
		PowerTable[i]  = log10 ((p1h + p2h) / (gsl_spline_eval (ellipSpline, kPowerTable[i], ellipAcc) + 1) /
		                        (gsl_spline_eval (alignSpline, kPowerTable[i], alignAcc) + 1));
	} //for
	...
	for (i      = 0; i < NK + extralin; ++ i) {
		...
		if (k < 4 * PI / Rhalo_cc) {
			...
			if (Rhalo_cs > 0) {
				pcorr_cs = min((mugal_qawo (k, 4 * PI / 3. * Delta * rho_mean * pow (Rhalo_cs, 3)) - TopHatWindow (k * Rhalo_cs / Delta_invth)) * 4 *
				               PI / 3. * pow (Rhalo_cs, 3) * (pconv_W (k, Rhalo_cs, 1) + TopHatWindow (k * Rhalo_cs)), TwoPowerSpec (k, 1));
			} else {
				pcorr_cs = 0;
			}
			if (Rhalo_ss > 0) {
				pcorr_ss = min((pow (mugal_qawo (k, 4 * PI / 3. * Delta * rho_mean * pow (Rhalo_ss, 3)), 2) -
				                pow (TopHatWindow (k * Rhalo_ss / Delta_invth), 2)) * 4 * PI / 3. * pow (Rhalo_ss, 3) *
				               (pconv_W (k, Rhalo_ss, 2) + TopHatWindow (k * Rhalo_ss)), TwoPowerSpec (k, 2));
			} else {
				pcorr_ss = 0;
			}
		} //if
			...
			if (Rhalo_cs > 0) {
				pcorr_cs = TwoPowerSpec (k, 1);
			} else {
				pcorr_cs = 0;
			}
			if (Rhalo_ss > 0) {
				pcorr_ss = TwoPowerSpec (k, 2);
			} else {
				pcorr_ss = 0;
			}
		} //else
...
double TwoPowerSpec (double k, int censat) {
	if (censat == 0) {
		if (k > 0.01) {
			...
			return
					pow (k, pow_ns) / pow (1e-4, pow_ns) * PowerSpec (1e-4) / PowerSpec (0.01) *
					pow (10., gsl_spline_eval (Twopow_ccSpline, - 2., Twopow_ccAcc));
		}
	} //if
	else if (censat == 1) {
		if (k > 0.01) {
			...
			return
					pow (k, pow_ns) / pow (1e-4, pow_ns) * PowerSpec (1e-4) / PowerSpec (0.01) *
					pow (10., gsl_spline_eval (Twopow_csSpline, - 2., Twopow_csAcc));
		}
	} //else if
	else {
		if (k > 0.01) {
			...
			return
					pow (k, pow_ns) / pow (1e-4, pow_ns) * PowerSpec (1e-4) / PowerSpec (0.01) *
					pow (10., gsl_spline_eval (Twopow_ssSpline, - 2., Twopow_ssAcc));
		}
	} //else
} //TwoPowerSpec
...
double calc_mean_rhalo_simple (int censat, int norm) {
	...
	F.params   = &params;
	gsl_integration_qag (&F, log (pow (10., cutoff_low[2 + censat])), log (pow (10., cutoff_high[2 + censat])), 0, 1.0e-3, WORKSIZE,
...
double mugal_qawo (double k, double m) {
	...
	double
	                         norm   =
			                         pc / (rvir3 * 4 * PI * exp (gsl_sf_lngamma (pa / pc) + log (gsl_sf_gamma_inc_P (pa / pc, pow (xmax / pb, pc)))));
...
double my_f (const gsl_vector*v, void*params) {
	...
	if (gsl_vector_get (v, 0) < pa_low) {
		a = pow (10., pa_low);
	} else if (gsl_vector_get (v, 0) > pa_high) {
		a = pow (10., pa_high);
	} else {
		a = pow (10., gsl_vector_get (v, 0));
	}
	if (gsl_vector_get (v, 1) < pb_low) {
		b = pow (10., pb_low);
	} else if (gsl_vector_get (v, 1) > pb_high) {
		b = pow (10., pb_high);
	} else {
		b = pow (10., gsl_vector_get (v, 1));
	}
	if (gsl_vector_get (v, 2) < pc_low) {
		c = pow (10., pc_low);
	} else if (gsl_vector_get (v, 2) > pc_high) {
		c = pow (10., pc_high);
	} else {
		c = pow (10., gsl_vector_get (v, 2));
	}
	tot       = numrad * (log (c) - a * log (b) - gsl_sf_lngamma (a / c));
	for (i    = 0; i < numrad; ++ i) {
		tot += (a - 3) * log (p[i]) - pow (p[i] / b, c);
	}
	return - tot;
} //my_f
...
void paramerror (double*x, double*p, double*perror) {
	...
	double dblderiv   = 0;
	for (i = 0; i < numrad; ++ i) {
		dblderiv += pow (a / c * polygamma0 - a * log (x[i] / b), 2);
	}
	perror[0] = pow (1. / (sqrt (2 * PI) * dblderiv), 1. / 3.);
	dblderiv = 0;
	for (i   = 0; i < numrad; ++ i) {
		dblderiv += pow (a - c * pow (x[i] / b, c), 2);
	}
