...
void add_galaxies_together (int t, int p, double deltaT) {
	...
	if (mass_ratio > ThreshMajorMerger) {
		...
		if (Gal[t].BulgeMass == 0.0) {
			Gal[t].CBMassMajor = Gal[p].BulgeMass + Gal[p].DiskMass;
		}
			// Major mergers on galaxies with CB through minor or major mergers or PB. New bulge is a CB through major
			// mergers.
		else if (Gal[t].BulgeMass > 0.0) {
			Gal[t].CBMassMajor = Gal[t].BulgeMass + Gal[p].BulgeMass + Gal[p].DiskMass;
			Gal[t].PBMass      = 0.0;
			Gal[t].CBMassMinor = 0.0;
		}
		transfer_material (t, "BulgeMass", p, "BulgeMass", 1., "model_mergers.c", __LINE__);
		transfer_material (t, "BulgeMass", p, "DiskMass", 1., "model_mergers.c", __LINE__);
	} else {
		// Minor mergers on bulge-less galaxies. New bulge is a CB through minor mergers.
		if (Gal[t].BulgeMass == 0.0) {
			Gal[t].CBMassMinor = Gal[p].BulgeMass + Gal[p].DiskMass;
			Gal[t].CBMass      = Gal[t].CBMassMinor;
		}
