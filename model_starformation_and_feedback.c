	...
	if ((Gal[Gal[p].CentralGal].HotGas > 0. && HotGasOnType2Galaxies == 0) ||
		...
		if (HotGasOnType2Galaxies == 0) {
			if (ejected_mass > Gal[Gal[p].CentralGal].HotGas)
				// Either eject own gas or merger_centre gas for type 2's
			{
...
void check_disk_instability (int p, double dt, double time) {
	...
	double Mcrit, UnstableMass, GasMass, GasRd, DiskMass, DiskRd, TotalMass, NewGasMass, NewDiskMass, NewDiskRd,
		   NewGasRd, NewTotalMass, Tester;
	double fraction, EpsilonGas, InverseEpsilonGas, EpsilonStars, InverseEpsilonStars, NewInverseEpsilonStars,
		   NewInverseEpsilonGas, EpsilonMin, EpsilonTot, InverseEpsilonTot;
		   #ifdef H2_AND_RINGS
	double mstarsRings[RNUM];
	int j;
	#endif // H2_AND_RINGS
	#else // DI_INSTABILITIES
	double Mcrit, fraction, UnstableMass, diskmass;
	...
	if (GasMass > 0.0 && DiskMass > 0.0) {
		...
		// Calculate Q parameters for gaseous and stellar disks.
		if (Gal[p].Type != 0) {
	...
	// Calculate the mass-weighted EpsilonTot parameter.
	InverseEpsilonTot = (1.1 * DiskMass) / (TotalMass * EpsilonStars) + (0.9 * GasMass) / (TotalMass * EpsilonGas);
	EpsilonTot        = 1.0 / InverseEpsilonTot;

	// Check the stability of the two-component disk.
	if (EpsilonTot < 1.0 && Tester == 2.0) {
		// If gaseous disk is stable (i.e., EpsilonGas >= EpsilonMin) then EpsilonStars solely has to raise EpsilonTot to 1.
		if (EpsilonGas >= EpsilonMin) {
			InverseEpsilonStars = (TotalMass * EpsilonGas - 0.9 * GasMass) / (1.1 * DiskMass * EpsilonGas);

			// Calculate disk's critical mass.
			if (Gal[p].Type != 0) {
				Mcrit = InverseEpsilonStars * InverseEpsilonStars *
						(Gal[p].InfallVmax * Gal[p].InfallVmax * DiskRd / G);
			} else {
			...
			// Invoke a function to deal with the unstable stars.
			if (UnstableMass > 0.0) {
				fix_stellar_disk_instability (p, UnstableMass, fraction);
			}
		} // if (EpsilonGas > EpsilonMin)

			// If stellar disk is stable (i.e., EpsilonStars > EpsilonMin) then EpsilonGas solely has to raise EpsilonTot to 1.
		else if (EpsilonStars >= EpsilonMin) {
			InverseEpsilonGas = (TotalMass * EpsilonStars - 1.1 * DiskMass) / (0.9 * GasMass * EpsilonStars);
			...
			// Then check the stability of the stellar disk again because new stars have been formed.
			NewGasMass   = Gal[p].ColdGas;
			NewDiskMass  = Gal[p].DiskMass;
			NewTotalMass = NewDiskMass + NewGasMass;
			NewDiskRd    = Gal[p].DiskRadius / 3.0;

			NewInverseEpsilonStars =
					(NewTotalMass / (1.1 * NewDiskMass)) * (1 - ((0.9 * NewGasMass) / (NewTotalMass *
																					   InverseEpsilonGas)));

			// Calculate disk's critical mass.
			if (Gal[p].Type != 0) {
				Mcrit = NewInverseEpsilonStars * NewInverseEpsilonStars *
						(Gal[p].InfallVmax * Gal[p].InfallVmax * NewDiskRd / G);
			} else {
				Mcrit = NewInverseEpsilonStars * NewInverseEpsilonStars * (Gal[p].Vmax * Gal[p].Vmax * NewDiskRd / G);
			}
			// Calculate the unstable stellar mass.
			...
			// If both disks are unstable then first raise the most unstable component to EpsilonMin and then adjust the other as necessary.
		else if (EpsilonStars < EpsilonMin && EpsilonGas < EpsilonMin) {
			if (EpsilonStars >= EpsilonGas) {
				// Calculate disk's critical mass.
				if (Gal[p].Type != 0) {
					Mcrit = (Gal[p].InfallVmax * Gal[p].InfallVmax * GasRd) / G;
				} else {
					Mcrit = (Gal[p].Vmax * Gal[p].Vmax * GasRd) / G;
				}
				// Calculate the unstable gaseous mass.
				UnstableMass = GasMass - Mcrit;
				...
				// Then check the stability of the stellar disk.
				NewGasMass   = Gal[p].ColdGas;
				NewDiskMass  = Gal[p].DiskMass;
				NewTotalMass = NewDiskMass + NewGasMass;
				NewDiskRd    = Gal[p].DiskRadius / 3.0;

				NewInverseEpsilonStars = (NewTotalMass / (1.1 * NewDiskMass)) * (1 - ((0.9 * NewGasMass) /
																					  NewTotalMass));
				...
				if (Gal[p].Type != 0) {
					...
					Mcrit = NewInverseEpsilonStars * NewInverseEpsilonStars *
							(Gal[p].Vmax * Gal[p].Vmax * NewDiskRd / G);
				}
				// Calculate the unstable stellar mass.
				UnstableMass           = Gal[p].DiskMass - Mcrit;
				fraction               = UnstableMass / Gal[p].DiskMass;
				...
				// Calculate disk's critical mass.
				if (Gal[p].Type != 0) {
					Mcrit = (Gal[p].InfallVmax * Gal[p].InfallVmax * DiskRd) / G;
				} else {
					Mcrit = (Gal[p].Vmax * Gal[p].Vmax * DiskRd) / G;
				}
				...
				// Then check the stability of the gaseous disk.
				NewGasMass   = Gal[p].ColdGas;
				NewDiskMass  = Gal[p].DiskMass;
				NewTotalMass = NewDiskMass + NewGasMass;
				NewGasRd     = Gal[p].ColdGasRadius / 3.0;

				NewInverseEpsilonGas = (NewTotalMass / (0.9 * NewGasMass)) * (1 - ((1.1 * NewDiskMass) / NewTotalMass));

				// Calculate disk's critical mass.
				if (Gal[p].Type != 0) {
					Mcrit = NewInverseEpsilonGas * NewInverseEpsilonGas * (Gal[p].InfallVmax * Gal[p].InfallVmax *
																		   NewGasRd / G);
				} else {
					Mcrit = NewInverseEpsilonGas * NewInverseEpsilonGas * (Gal[p].Vmax * Gal[p].Vmax * NewGasRd / G);
				}
				// Calculate the unstable gaseous mass.
				UnstableMass         = GasMass - Mcrit;
		...
		} // if (EpsilonStars < EpsilonMin && EpsilonGas < EpsilonMin)
	} // if (EpsilonTot < 1.0 && Tester == 2.0)
	else if (Tester == 1.0) {
		if (GasMass <= 0.0) {
			if (Gal[p].Type != 0) {
				Mcrit = (Gal[p].InfallVmax * Gal[p].InfallVmax * (Gal[p].DiskRadius / 3.0)) / (1.1 * 1.1 * G);
			} else {
				Mcrit = (Gal[p].Vmax * Gal[p].Vmax * (Gal[p].DiskRadius / 3.0)) / (1.1 * 1.1 * G);
			}
			// Calculate the unstable stellar mass.
			UnstableMass = Gal[p].DiskMass - Mcrit;
			fraction     = UnstableMass / Gal[p].DiskMass;
			...
			// Calculate disk's critical mass.
			if (Gal[p].Type != 0) {
				Mcrit = (Gal[p].InfallVmax * Gal[p].InfallVmax * (Gal[p].ColdGasRadius / 3.0)) / (0.9 * 0.9 * G);
			} else {
				Mcrit = (Gal[p].Vmax * Gal[p].Vmax * (Gal[p].ColdGasRadius / 3.0)) / (0.9 * 0.9 * G);
			}
			// Calculate the unstable gaseous mass.
			UnstableMass = GasMass - Mcrit;
...
//		}
//
//		//	Check which disk has larger Q since RW11 formula gives less weight to that component.
//		EpsilonTot = sqrt ((EpsilonStars * EpsilonStars * EpsilonGas * EpsilonGas) / (EpsilonStars * EpsilonStars + EpsilonGas * EpsilonGas));
//
//		// Check the stability of the two-component disk. Follow RW11 calculations and assume that EpsilonGas/EpsilonStars ~ 1
//		// which suggests that, on average, stars and gas contribute equally to the gravitational instability of the disc.
//		// So we can define EpsilonMin = 1 + W as a solution to the inequality EpsilonGas * EpsilonStars / EpsilonGas + EpsilonStars >= 1.
//		if (EpsilonTot < 0.999)
//		{
//			printf ("EpsilonTot=%e EpsilonStars=%e EpsilonGas=%e \n", EpsilonTot, EpsilonStars, EpsilonGas);
//		}
//		///////////////////////////////////////////////////////// CHECK ///////////////////////////////////////////////////////
	#else // DI_INSTABILITIES
...
// Check stellar disk -> eq 34 Guo2010
	...
	if (UnstableMass > 0.0) {
		...
		// The bulge will be formed in the same place as the disk was, so the disk rings are transferred directly
		// into bulge rings.
#ifdef H2_AND_RINGS
...
// represents the instability-driven bulge mass.
		// Instabilities on bulge-less galaxies. New bulge is a PB.
		if (Gal[p].BulgeMass == 0.0) {
			Gal[p].PBMass = UnstableMass;
		}
			// Instabilities on galaxies with CB through minor mergers or PB. New bulge is a PB.
		else if (Gal[p].BulgeMass > 0.0 && Gal[p].CBMassMajor == 0.0) {
			Gal[p].PBMass      = Gal[p].BulgeMass + UnstableMass;
			Gal[p].CBMass      = 0.0;
			Gal[p].CBMassMinor = 0.0;
		}
			// Instabilities on galaxies with CB through major mergers. New bulge is a CB through major mergers plus a PB
		else if (Gal[p].CBMassMajor > 0.0) {
			Gal[p].PBMass += UnstableMass;
		}
...
void fix_gaseous_disk_instability (int p, double GasMass, double UnstableMass, double dt, double time) {
	...
	if (Gal[p].BlackHoleMass > 0.0) {
		...
		transfer_material (p, "BlackHoleMass", p, "ColdGas", BHfraction, "model_starformation_and_feedback.c",
		                   __LINE__);
...
void update_bulgesize_from_stellardisk_instability (int p, double UnstableMass) {
	...
	massfrac  = UnstableMass / bulgemass;
	...
	diskmass = Gal[p].DiskMass;
	massfrac = UnstableMass / diskmass;
	...
	if (Gal[p].BulgeMass < TINY_MASS) {
		...
		Gal[p].BulgeSize = (Gal[p].BulgeMass + UnstableMass) * (Gal[p].BulgeMass + UnstableMass) /
		                   (Gal[p].BulgeMass * Gal[p].BulgeMass / Gal[p].BulgeSize +
		                    UnstableMass * UnstableMass / bulgesize +
	...
	#ifdef BULGESIZE_DEBUG
	if ((Gal[p].BulgeMass + UnstableMass > TINY_MASS && Gal[p].BulgeSize < TINY_LENGTH)
		|| (Gal[p].BulgeMass + UnstableMass < TINY_MASS && Gal[p].BulgeSize > TINY_LENGTH)) {
		printf("Original DiskMass=%e, DiskSize=%e\nOriginal BulgeMass=%e, BulgeSize=%e\nTransferred UnstableMass=%e,"
					   "bulgesize=%e\nFinal BulgeMass=%e, BulgeSize=%e\n", Gal[p].DiskMass, Gal[p].DiskRadius,
			   Gal[p].BulgeMass, orisize, UnstableMass, bulgesize, Gal[p].BulgeMass + UnstableMass, Gal[p].BulgeSize);
