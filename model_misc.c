...
double get_initial_disk_radius (int halonr, int p) {
	...
	if (DiskRadiusModel == 0 || DiskRadiusModel == 1) {
		...
		if (Halo[halonr].Spin[0] == 0 && Halo[halonr].Spin[1] == 0 && Halo[halonr].Spin[2] == 0) {
			...
			dgas = 3 * sqrt (Halo[halonr].Spin[0] * Halo[halonr].Spin[0] +
...
double get_gas_disk_radius (int p) {
	...
	if ((Gal[p].ColdGasSpin[0] == 0 && Gal[p].ColdGasSpin[1] == 0 && Gal[p].ColdGasSpin[2] == 0) ||
	    Gal[p].ColdGas == 0.) {
...
double get_stellar_disk_radius (int p) {
	...
	//if the spin is not available for the Disk
...
#define RADIUS_N 100
	...
	// increases the search radius until it encompasses half the total mass taking into account the stellar disk,
	// stellar bulge and cold gas disk.
	ii = 0;
	do {
...
#define SAT_RADIUS_N 100
	...
	// increases the search radius until it encompasses half the total light taking
...
double stellar_ninety_light_radius (struct GALAXY_OUTPUT*o) {
	double r, rd, rb, L;
	int    ii;
	double Ldisk, Lbulge, totL;
	double rmax, dr;
...
#define SAT_RADIUS_N 100
	...
	// increases the search radius until it encompasses 90pc of the total light taking
	// into account the stellar disk, stellar bulge and cold gas disk.
	ii = 0;
	do {
		L = 0.;
		...
		ii ++;
		if (ii > N) terminate("couldn't find 90pc light radius");
	} while (L < 0.9 * totL);
...
void init_galaxy (int p, int halonr) {
	...
	Gal[p].XrayLum       = 0.0;
	Gal[p].ColdGasRadius = get_initial_disk_radius (halonr, p); // not invoked, see main.c
...
void transfer_material (int p, char cp[], int q, char cq[], double fraction, char call_function[], int call_line) {
	...
	float  Mass;
	double mi, ma;
	double mass_ratio, pmass_total, qmass_total;

	// Calculate mass ratio of the galaxies
	pmass_total = Gal[p].DiskMass + Gal[p].BulgeMass + Gal[p].ColdGas;
	qmass_total = Gal[q].DiskMass + Gal[q].BulgeMass + Gal[q].ColdGas;

	if (max(pmass_total, qmass_total) > 0.) {
		mass_ratio = min(pmass_total, qmass_total) / max(pmass_total, qmass_total);
	} else {
		mass_ratio = 1.0;
	}
#ifdef DETAILED_METALS_AND_MASS_RETURN
   ...
   float dObsYLum[NMAG][NOUT];
	...
	if (strcmp (cp, "ColdGas") == 0) {
		...
		for(outputbin = 0; outputbin < NOUT; outputbin++)
	  {
		...
		// Minor mergers on bulge-dominated galaxies.
		// Storing the stars that are transferred from the disk of the satellite to the bulge of the central
		if ( strcmp(cq, "DiskMass") == 0 ) {
			Gal[p].CBMass += Mass;
			if ( mass_ratio < ThreshMajorMerger ) {
				Gal[p].CBMassMinor += Mass;
			}
			// Major mergers.
			// Avoid storing mass when check_disk_instability is invoked. The CBMassMajor variable contains also
			// mass of merger-induced starburst which contributes to the merger-driven bulge mass. We only care for major
			// mergers because that's the only case where stars go to the bulge, in minor merger those stars stay on the
			// disk.
			if ( mass_ratio > ThreshMajorMerger && strcmp(call_function, "model_mergers.c") == 0 ) {
				Gal[p].CBMassMajor += Mass;
			}
		}
#else // DI_MERGERS
...
//			}
//		}
////		 Storing the stars that are transferred from the disk of the satellite to the bulge of the central. Avoid
////		 storing mass when check_disk_instability is invoked.
//		if (strcmp (cq, "DiskMass") == 0 && strcmp (call_function, "model_mergers.c") == 0) {
//			Gal[p].CBMass += Mass;
//			if (mass_ratio > ThreshMajorMerger) {
//				Gal[p].CBMassMajor += Mass;
//			} else {
