	...
	while ( prog >= 0 ) {
		...
		for ( i = 0, currentgal = HaloAux[prog].FirstGalaxy; i < HaloAux[prog].NGalaxies; i ++ ) {
			if ( ngal >= MaxGal ) {
				...
				if ( MaxGal < ngal + 1 )
					MaxGal = ngal + 1;  // How can this ever be true?
...
void evolve_galaxies( int halonr, int ngal, int treenr ) {
	...
	if ( Gal[FOF_centralgal].Type != 0 || Gal[FOF_centralgal].HaloNr != halonr ) terminate(
	...
	for ( nstep = 0; nstep < STEPS; nstep ++ ) {
		...
		if ( BlackHoleGrowth == 1 )
			for ( p = 0; p < ngal; p ++ ) {
				AGNaccreted = min(Gal[p].BlackHoleGas,
				                  Gal[p].BlackHoleMass * BlackHoleAccretionRate * deltaT / (STEPS * t_Edd));
				if ( AGNaccreted > 0. ) {
