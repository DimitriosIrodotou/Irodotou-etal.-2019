...
void check_disk_instability (int p, double dt, double time)
{
	...
	double fraction, EpsilonGas, EpsilonStars, EpsilonMin, EpsilonTotal;
	#ifdef H2_AND_RINGS
	...
	if (GasMass > 0.0 && DiskMass > 0.0)
	{
		...
		EpsilonTotal = sqrt((EpsilonStars * EpsilonStars * EpsilonGas * EpsilonGas) / (EpsilonStars * EpsilonStars + EpsilonGas * EpsilonGas));
	}
	else
	{
		EpsilonMin = 1.0;
	...
	if (EpsilonTotal < 1.0 && EpsilonMin == 2.0)
	{
		// If gaseous disk is stable (i.e., EpsilonGas > EpsilonMin) then EpsilonStars solely has to raise EpsilonTotal to 1.
		if (EpsilonGas * EpsilonGas >= EpsilonMin)
		{
			// Calculate disk's critical mass.
			if (Gal[p].Type != 0)
			{
				Mcrit = (Gal[p].InfallVmax * Gal[p].InfallVmax * DiskRd * (EpsilonGas * EpsilonGas - 1.0))  / (G * EpsilonGas * EpsilonGas);
			}
			else
			{
				Mcrit = (Gal[p].Vmax * Gal[p].Vmax * DiskRd * (EpsilonGas * EpsilonGas - 1.0))  / (G * EpsilonGas * EpsilonGas);
			}
			...
			// If stellar disk is stable (i.e., EpsilonStars^2 > EpsilonMin) then EpsilonGas solely has to raise EpsilonTotal to 1.
		else if (EpsilonStars >= EpsilonMin)
		{
			// Store these values here before you update the gas and stellar disks
			DiskMass     = Gal[p].DiskMass;
			DiskRd       = Gal[p].DiskRadius / 3.0;
			...
			if (Gal[p].Type != 0)
			{
				Mcrit = (Gal[p].InfallVmax * Gal[p].InfallVmax * GasRd * (EpsilonStars * EpsilonStars - 1.0)) / (G * EpsilonStars * EpsilonStars);
			}
			else
			{
				Mcrit = (Gal[p].Vmax * Gal[p].Vmax * GasRd * (EpsilonStars * EpsilonStars - 1.0)) / (G * EpsilonStars * EpsilonStars);
			}
...
//			// Calculate disk's critical mass.
//			Mcrit = ((Gal[p].DiskRadius / 3.0) * DiskMass) / DiskRd;
//
//			// Calculate the unstable stellar mass.
...
//			}
			...
			// Both disks are unstable so first raise EpsilonGas to EpsilonMin and then EpsilonStars to EpsilonMin.
		else if (EpsilonStars * EpsilonStars < EpsilonMin && EpsilonGas * EpsilonGas < EpsilonMin)
		{
			// Calculate disk's critical mass.
			if (Gal[p].Type != 0)
			{
				Mcrit = (Gal[p].InfallVmax * Gal[p].InfallVmax * GasRd) / (G * EpsilonMin);
			}
			else
			{
				Mcrit = (Gal[p].Vmax * Gal[p].Vmax * GasRd) / (G * EpsilonMin);
			}
			// Calculate the unstable gaseous mass.
			UnstableMass = GasMass - Mcrit;
			...
			if (Gal[p].Type != 0)
			{
				Mcrit = (Gal[p].InfallVmax * Gal[p].InfallVmax * (Gal[p].DiskRadius / 3.0)) / (G * EpsilonMin);
			}
			else
			{
				Mcrit = (Gal[p].Vmax * Gal[p].Vmax * (Gal[p].DiskRadius / 3.0)) / (G * EpsilonMin);
			}
			// Calculate the unstable stellar mass.
			UnstableMass = Gal[p].DiskMass - Mcrit;
			fraction     = UnstableMass / Gal[p].DiskMass;
