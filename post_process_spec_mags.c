...
#endif
{
	...
	for (nlum = 0; nlum < NMAG; nlum ++) {
		...
		for (ll      = 0; ll <= o->sfh_ibin; ll ++) {
			...
			// Compute the time at beginning and end of SFH bin, divide into 10Myr steps. All the SFH bins are divided
			// into this smaller steps for the LUM calculation
			...
			for (N_AgeBin = 1; N_AgeBin <= MaxBins; N_AgeBin ++) {
				// Define N_AgeBin
				if (bin_size / ((float) N_AgeBin) <= Small_Age_bin) {
					break;
				}
				if (N_AgeBin == MaxBins) terminate("MaxBins reached in post_process_mags");
			}
			//#else
			//N_AgeBin=1;
			//#endif
		...
		if ((o->DiskMass + o->BulgeMass) > 0.0 && nlum == 17) {
			...
			o->rbandWeightAge = o->rbandWeightAge / 1000. * UnitTime_in_Megayears /
			                    Hubble_h; //conversion in age from code units/h -> Gyr
		}
...
void compute_post_process_lum (double mass, double age, double metals, int snap, int nlum,
	...
	// Time below which the luminosities are corrected for extinction due to molecular birth clouds.
...
void dust_correction_for_post_processing (int nlum, int snap, double Zg, double ColdGas, double ColdGasRadius,
	...
	/* 0.94 = 2.83/3. - 3 to get scale length and 2.83 = 1.68^2 */
	...
	// mu for YS extinction, given by a Gaussian with centre 0.3 (MUCENTER) and width 0.2 (MUWIDTH), truncated at 0.1 and 1.
	mu = - 1.;
	while (mu < 0) {
	...
	*LumDiskDust = LumDisk * alam;
	...
	// Remove light from young stars absorbed by birth clouds
