...
void compute_cooling (int p, double dt, int ngal) {
	...
	if (tot_hotMass > 1.0e-6) {
		...
		// temp -> Temperature of the Gas in Kelvin, obtained from hidrostatic equilibrium KT=0.5*mu_p*(Vc)^2 assuming
		// Vvir~Vc
		temp = 35.9 * Vvir * Vvir;
	...
	for (p = 0; p < ngal; p ++) {
		...
		if (HotGas > 0.0) {
				...
				// Henriques 2015.  Proportional to MBH * MHotgas
			{
				...
				// AGNrate = AgnEfficiency * (UnitTime_in_s*SOLAR_MASS)/(UnitMass_in_g*SEC_PER_YEAR)
				// *pow(Gal[p].BlackHoleMass,0.5)*(Gal[p].HotGas * Gal[p].Rvir/Gal[p].HotRadius)*0.1;*/ else AGNrate =0.
				...
				// temp -> Temperature of the Gas in Kelvin, obtained from hidrostatic equilibrium KT=0.5*mu_p*(Vc)^2
				// assuming Vvir~Vc
				temp   = 35.9 * Vvir * Vvir;
				if (tot_metals > 0) {
				...
				// Bondi-Hoyle accretion recipe -- efficiency = 0.15 Eq. 29 in Croton 2006
				if (AGNRadioModeModel == 2) {
					AGNrate = (2.5 * M_PI * G) * (0.75 * 0.6 * x) * Gal[p].BlackHoleMass * 0.15;
				} else if (AGNRadioModeModel == 3) {
					// Cold cloud accretion recipe -- trigger: Rff = 50 Rdisk, and accretion rate = 0.01% cooling rate
			...
			// Eddington rate Note that this assumes an efficiency of 50% - it ignores the e/(1-e) factor in L = e/(1-e)
			// Mdot c^2 */
			...
			// Accreted mass onto black hole the value of dt puts an h factor into AGNaccreted as required for code
			// units
			...
			//  coefficient to heat the cooling gas back to the virial temperature of the halo 1.34e5 = sqrt(2*eta*c^2),
			// eta=0.1 (standard efficiency) and c in km/s Eqs. 11 & 12 in Croton 2006
		...
		/*if(Gal[p].ReheatedGas>0.)
	  {
		...
		if (DiskRadiusModel == 0) {
			...
			if (Gal[p].ColdGas > 0.0) {
				for (ii = 0; ii < 3; ii ++) {
#ifdef DI_GASSPIN
					Gal[p].ColdGasSpin[ii] =
							(Gal[p].ColdGasSpin[ii] * Mdisk +  f * Halo[Gal[p].HaloNr].Spin[ii] * Mcool) /
							(Gal[p].ColdGas);
#else // DI_GASSPIN
					Gal[p].ColdGasSpin[ii] =
							(Gal[p].ColdGasSpin[ii] * Mdisk + Halo[Gal[p].HaloNr].Spin[ii] * Mcool) /
							(Gal[p].ColdGas);
#endif // DI_GASSPIN
				}
