	...
	for (snap = 0; snap < (LastDarkMatterSnapShot + 1); snap ++) {
		...
		for (step = 0; step < STEPS; step ++) {
			...
			if (snap == 0 && step == 0) {
				...
				ibin += 1;
				if (ibin == SFH_NBIN) terminate("sfh_update_bins: too many bins required\n");
			...
			while (flag_merged_bins) {
				...
				while (! flag_merged_bins && dt_merge > 0) {
					...
					int itemp;
					itemp  = i;
					for (i = itemp; sfh_Nbins[i] == dt_merge; i ++) {
						n_merge += 1;
					}
			...
			for (j               = 0; j <= sfh_ibin; j ++) {
				...
				if (j == 0) {
					...
					SFH_dt[snap][step][j] = sfh_t[j - 1] - sfh_t[j];
