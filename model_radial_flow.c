...
void gas_inflow (int p, double time) {
	...
	if (Gal[p].ColdGas < 1.0e-6) {
		...
		for (j = 1; j < RNUM; j ++) {
			rgas += (0.5 * (RingRadius[j - 1] + RingRadius[j]) * Gal[p].ColdGasRings[j]);
		}
	...
	for (j = 0; j < RNUM; j ++) {
		...
		if (r_out <= RingRadius[0]) {
			...
			for (index = 0; RingRadius[index] <= r_out && index < RNUM - 1; index ++) {
			}    //ring number of the r_out
			index --;
			r1                                   = RingRadius[index];
			r2                                   = r_out;
			while (r1 >= r_in) {
