...
void init_numgal (float masslimit_low, float masslimit_high, int snap) {
	...
	for (mbin = 0; mbin < massbins; ++ mbin) {
		for (j = 0; j < NFofsInSample[snap]; ++ j) {
			...
			if (MCMC_FOF2[j].M_Crit200[snap] > minfofmass + mbin * (maxfofmass - minfofmass) / (double) massbins &&
				...
				if (MCMC_GAL[HashTable[i]].log10_StellarMass[snap] > masslimit_low &&
				    MCMC_GAL[HashTable[i]].log10_StellarMass[snap] <= masslimit_high) {
					ncen = 1;
				}

				for (k = 1; k < MCMC_GAL[HashTable[i]].ngal[snap]; ++ k) {
					if (MCMC_GAL[HashTable[i + k]].log10_StellarMass[snap] > masslimit_low &&
					    MCMC_GAL[HashTable[i + k]].log10_StellarMass[snap] <= masslimit_high) {
						nsat ++;
					}
				} //for
				usedbymass[mbin] ++;
			...
			if (MCMC_FOF2[j].M_Crit200[snap] < minfofmass2 && nsat > 0) {
				minfofmass2 = MCMC_FOF2[j].M_Crit200[snap];
			}
			if (MCMC_FOF2[j].M_Crit200[snap] > maxfofmass2 && nsat > 0) {
				maxfofmass2 = MCMC_FOF2[j].M_Crit200[snap];
			}
		} //for (loop on FoFID)
	...
	for (j = 0; j < NFofsInSample[snap]; ++ j) {
		...
		if (mbin2 == NMassTable2) {
			mbin2 --;
		} //because of maxfofmass2/roundoff
		...
		if (mbin2 >= 0 && mbin2 < NMassTable2 && MCMC_GAL[HashTable[i]].ngal[snap] > 1) {
			...
			for (jj = 1; jj < MCMC_GAL[HashTable[i]].ngal[snap]; ++ jj) {
				if (MCMC_GAL[HashTable[i + jj]].log10_StellarMass[snap] > masslimit_low &&
					...
					//		MCMC_GAL[HashTable[i]].fofid[snap], MCMC_GAL[HashTable[i+jj]].Type[snap], MCMC_GAL[HashTable[i+jj]].log10_StellarMass[snap]);
	...
	for (mbin2 = 0; mbin2 < NMassTable2; ++ mbin2) {
		...
		for (rbin = 0; rbin < NRadiusTable; ++ rbin) {
			if (NProfileTable[mbin2][rbin] > 0) {
				...
				ProfileTable[mbin2][rbin]    = (double) NProfileTable[mbin2][rbin] / (NgalTotal[mbin2] * 4. / 3. * PI *
				                                                                      (pow (10., 3 * ((rbin + 1) * rbinsize - 2.5)) -
				                                                                       pow (10., 3 * (rbin * rbinsize - 2.5))) *
				                                                                      pow (10., MassTable2[mbin2] + massoffset));
				ProfileErrTable[mbin2][rbin] = sqrt (NProfileTable[mbin2][rbin]) / (NgalTotal[mbin2] * 4. / 3. * PI *
				                                                                    (pow (10., 3 * ((rbin + 1) * rbinsize - 2.5)) -
				                                                                     pow (10., 3 * (rbin * rbinsize - 2.5))) *
				                                                                    pow (10., MassTable2[mbin2] + massoffset));
			} //if
		...
		if (NValid > 2) {
			...
			for (rbin = 0; rbin < NRadiusTable; ++ rbin) {
				if (NProfileTable[mbin2][rbin] > 0) {
					...
					weights[i] = max(1. / sqrt (MassVarTable2[mbin2] + SQR (0.5 * (log10 (ProfileTable[mbin2][rbin] + ProfileErrTable[mbin2][rbin]) -
					                                                               log10 (ProfileTable[mbin2][rbin] -
					                                                                      ProfileErrTable[mbin2][rbin])))), 1. /
					                                                                                                        sqrt (MassVarTable2[mbin2] +
					                                                                                                              SQR (3 *
					                                                                                                                   (log10 (ProfileTable[mbin2][rbin] +
					                                                                                                                           ProfileErrTable[mbin2][rbin]) -
					                                                                                                                    log10 (ProfileTable[mbin2][rbin])))));
...
int einastofit (int m, int n, double*p, double*dy, double**dvec, void*vars) {
	int           i;
	double        lognorm = - ((2. - 3. / pow (10., p[0])) * log10 (2.) + (2. / pow (10., p[0])) / log (10.) + log10 (PI) +
	                           3 * log10 (pow (10., p[1])) + (3. / pow (10., p[0]) - 1) * log10 (pow (10., p[0])) +
	                           gsl_sf_lngamma (3. / pow (10., p[0])) / log (10.) +
	                           log10 (gsl_sf_gamma_inc_P (3. / pow (10., p[0]), 2 * pow (pow (10., p[1]), - pow (10., p[0])) / pow (10., p[0]))) +
	                           fitconst);
...
double mugal_qawo (double k, double m) {
	...
	double                   norm   = 1. / (rvir3 * PI *
	                                        exp ((2. - 3. / alpha) * log (2.) + 2. / alpha + 3 * log (rs) + (3. / alpha - 1) * log (alpha) +
	                                             gsl_sf_lngamma (3. / alpha) + log (gsl_sf_gamma_inc_P (3. / alpha, 2 * pow (rs, - alpha) / alpha))));
...
double alpha_eval (double m) {
	double value;
	if (m < alpharscutoff_low) {
		return gsl_spline_eval (alphaSpline, alpharscutoff_low, alphaAcc);
	} else if (m > alpharscutoff_high) {
		return gsl_spline_eval (alphaSpline, alpharscutoff_high, alphaAcc);
	} else {
...
double rs_eval (double m) {
	double value;
	if (m < alpharscutoff_low) {
		return gsl_spline_eval (rsSpline, alpharscutoff_low, rsAcc);
	} else if (m > alpharscutoff_high) {
		return gsl_spline_eval (rsSpline, alpharscutoff_high, rsAcc);
	} else {
...
double Mcensat (double k, int i, int j) { //k=wavenumber, i=which haloterm [0/1], j=central/satellite [0/1/2]
	...
	F.params   = &params;
	gsl_integration_qag (&F, log (pow (10., cutoff_low[3 - i + j])), log (pow (10., cutoff_high[3 - i + j])), 0, 1.0e-3, WORKSIZE, GSL_INTEG_GAUSS41,
	                     w, &result, &abserr);
	gsl_integration_workspace_free (w);
	return result;
} //Mcensat
...
double ngal_mean_calc (int j) {
	...
	F.params   = &params;
	gsl_integration_qag (&F, log (pow (10., cutoff_low[j])), log (pow (10., cutoff_high[j])), 0, 1.0e-3, WORKSIZE, GSL_INTEG_GAUSS41, w, &result,
	                     &abserr);
	gsl_integration_workspace_free (w);
	return result;
} //ngal_mean_calc
